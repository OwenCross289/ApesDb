...
public sealed class GetGamesEndpoint : Endpoint<GetGamesRequest, Pagable<GetGameResponse>>
{
    ...
    public GetGamesEndpoint(ApplicationDbContext dbContext)
    {
    ...
    {
        Get(ApiRoutes.Games.Get);
        Summary(summary =>
        {
            summary.Summary = "Gets and filters synchronized games.";
            summary.Description = "Returns an alphabetized, paginated view of the synchronized game catalog.";
        });
    }

    public override async Task HandleAsync(GetGamesRequest request, CancellationToken ct)
    {
        ...
        await Send.OkAsync(
            new Pagable<GetGameResponse>(items, total, filteredTotal, request.Page, request.PageSize),
            ct
        );
    }
    ...
    private Task<GetGameResponse[]> Query(IQueryable<Game> query, GetGamesRequest request, CancellationToken ct)
    {
        return query
