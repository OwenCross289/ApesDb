...
import type { GameAddon, GameDetails, GameReference } from "./game-details.schemas";
...
function AddonCover({ addon }: { addon: GameAddon }) {
  const coverUrl = addon.coverSmallUrl ?? addon.coverLargeUrl;
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        {addons.map((addon) => (
          <Link
            ...
            key={addon.id}
            params={{ gameId: addon.id.toString() }}
            to="/games/$gameId"
          >
            <AddonCover
              key={addon.coverSmallUrl ?? addon.coverLargeUrl ?? "missing"}
              addon={addon}
            />
            <div className="min-w-0 py-1">
              <Badge variant="secondary">{addon.type}</Badge>
              <h3 className="mt-2 font-medium group-hover:underline group-hover:underline-offset-4">
                {addon.name}
              </h3>
              {addon.releaseDate && (
                <p className="mt-1 text-xs text-muted-foreground">
                  {formatDate(addon.releaseDate)}
                </p>
              )}
              {addon.description && (
